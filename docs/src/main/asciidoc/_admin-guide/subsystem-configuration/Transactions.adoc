...
After subsystem configures attribute `statistics-enabled` then Narayana starts gathering
statistics about transaction processing. User can see these statistics by listing
particular attribute or list all statistics attributes as a group.
Transaction statistics attributes are runtime only attributes.

.printing all transaction statistics attributes
[source,sh]
----
